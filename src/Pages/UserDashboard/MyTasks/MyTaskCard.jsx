...
function MyTaskCard() {
  ...
  return (
    ...
    <div className='flex items-center justify-center sm:justify-start flex-wrap xl:flex-nowrap gap-4  scrollbar-none overflow-x-scroll '>
       ...
       < TaskDataCard handleClick={()=>{handleShowPopup(index)}} title={'MOBILE APP'} value1={'2'} value2={'4'} bgcolor={'bg-[#efeeff]'} textcolor={'text-[#5046e4]'} description={'Grocery Mobile App'}/>
       < TaskDataCard handleClick={()=>{handleShowPopup(index)}} title={'LANDING PAGE'} value1={'5'} value2={'8'} bgcolor={'bg-[#fff7ea]'} textcolor={'text-[#f4a012]'} description={'MR Corporation Website'}/>
       < TaskDataCard handleClick={()=>{handleShowPopup(index)}} title={'DASHBOARD'} value1={'3'} value2={'6'} bgcolor={'bg-[#ffebf5]'} textcolor={'text-[#e94a9b]'} description={'Grocery Dashboard'}/>
       < TaskDataCard handleClick={()=>{handleShowPopup(index)}} title={'MOBILE APP'} value1={'2'} value2={'4'} bgcolor={'bg-[#efeeff]'} textcolor={'text-[#5046e4]'} description={'Grocery Mobile App'}/>
       {popupCardIndex === index && showPopup && <GroceryDialog popupCardIndex={popupCardIndex} index={index} show={showPopup} onClose={handleClosePopup} />}
        ...
        <div className=' flex items-center justify-center mb-[40px]'>
            <button onClick={()=>{handleShowTaskPopup(index)}} className='flex items-center justify-center gap-[14px] px-[16px] py-[12px] w-[258px] border-[1px] border-[#fb6d72] border-dashed rounded-lg text-sm font-semibold font-public-sans bg-btn-gradient text-transparent bg-clip-text'><Svgs.AddIconRed/>Add New Task</button>
